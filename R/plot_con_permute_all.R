...
plot_con_permute_all<-function(alloutputs,top_pm,lgndcol=2,legend=FALSE,psigtitle,psigyrange=NULL,savepsigfile,psigpicdim,pvtitle,pvyrange=NULL,savepvfile,pvpicdim,estitle,esyrange=NULL,saveesfile,espicdim,crtitle,cryrange=NULL,savecrfile,crpicdim)
{
  require(ggplot2)
  ...
  else
  {
    ...
    ggplot(df, aes(x=dif, y=median)) +
      ...
      geom_line(size=1)+
      xlab("Proportion of permutations") +
  ...
  else
  {
    ...
    p<-ggplot(data=df,
      ...
      ylim(y_range)+
      xlab("Proportion of permutations") +
      ylab(paste("P-values of top", paste(top_pm, "important features", sep=" "), sep=" "))  +
      ggtitle(pvtitle) +
      theme_bw()+
      theme(plot.title = element_text(size = 30, face = "bold"),
  ...
  else
  {
    ...
    ggplot(data=df,
      ...
      ylim(y_range)+
      xlab("Proportion of permutations") +
      ylab(paste("Correlations of top", paste(top_pm, "important features", sep=" "), sep=" "))  +
      ggtitle(crtitle) +
      theme_bw()+
      theme(plot.title = element_text(size = 30, face = "bold"),
            axis.line = element_line(colour = "black"),
    ...
    ggsave(filename=savecrfile,plot=p,width = crpicdim[1], height = crpicdim[2])
  }
}
