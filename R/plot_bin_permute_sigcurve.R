plot_bin_permute_sigcurve<-function(alloutputs,samsize,lgndcol=2,psigtitle,savepsigfile,psigpicdim)
{
  require(ggplot2)
  ...
  else
  {
    ...
    p<-ggplot(df, aes(x=dif, y=prop)) +
      ...
      geom_ribbon(aes(ymin=prop, ymax=seq(prop[1],prop[len],length.out=len)),alpha=0.2, fill = "green")+
      geom_ribbon(aes(ymin=seq(prop[1],prop[len],length.out=len), ymax=1),alpha=0.2, fill = "gold")+
    ...
    ggsave(filename=savepsigfile,width = psigpicdim[1], plot=p, height = psigpicdim[2],device=cairo_ps)
  
    return(list(auc=auc,aoi=aoi,slope=slope,df=df, p=p))
  }
}