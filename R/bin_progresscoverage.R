bin_progresscoverage<-function(alloutputs,lowindx,top_pm,lgndcol=2,pvtitle,savepvfile,pvpicdim,estitle,saveesfile,espicdim)
{
  library(ggplot2)
  ...
  #lowindx<-which.min(df$median)-1
  trueorg<-df[df$dif==0,"median"]
  nsig_prop<- sum(df$q1[2:lowindx]<=trueorg & trueorg<=df$q2[2:lowindx])/lowindx
  ...
  for(i in 1:(N-1))
  {
    ...
    pv_cover[i,]<-as.numeric(as.character(df[df$dif==i,"q1"]))<=trueorg & trueorg<= as.numeric(as.character(df[df$dif==i,"q2"]))
  }
  ...
  for(i in 1:(N-1))
  {
    ...
    es_cover[i,]<-as.numeric(as.character(df[df$dif==i,"q1"]))<=trueorg & trueorg<= as.numeric(as.character(df[df$dif==i,"q2"]))
  }
